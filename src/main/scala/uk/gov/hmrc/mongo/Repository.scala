...
trait Repository[A <: Any, ID <: Any] extends CurrentTime {
  ...
  def removeAll(writeConcern: WriteConcern = WriteConcern.Default)(implicit ec: ExecutionContext): Future[WriteResult with WriteResultErrorFlag] = ???

  def removeById(id: ID, writeConcern: WriteConcern = WriteConcern.Default)(implicit ec: ExecutionContext): Future[WriteResult with WriteResultErrorFlag] = ???

  def remove(query: (scala.Predef.String, play.api.libs.json.Json.JsValueWrapper)*)(implicit ec: ExecutionContext): Future[WriteResult with WriteResultErrorFlag] = ???

  def drop(implicit ec: ExecutionContext): Future[Boolean] = ???

  def save(entity: A)(implicit ec: ExecutionContext): Future[WriteResult with WriteResultErrorFlag] = ???

  def insert(entity: A)(implicit ec: ExecutionContext): Future[WriteResult with WriteResultErrorFlag] = ???
